...
import { Button, message, Input, Tabs, Modal, Tag, Popconfirm } from 'antd';
import {
...
export default function KnowledgeDetail() {
  ...
  const { knowledgeList, toggleLike, toggleFavorite, addComment, deleteComment, archiveKnowledge, unarchiveKnowledge, softDeleteKnowledge, rollbackVersion, user } = useAppStore();
    ...
    return (
      <div className="page-container">
        <div className="empty-state">
          ...
          <div className="empty-text">知识不存在或已被删除</div>
    ...
    <>
      ...
      <div className="detail-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <span className={`kc-type-badge ${latestKnowledge.type}`}>
            {latestKnowledge.typeLabel}
          </span>
      ...
      <div className="detail-actions">
        <Button
          icon={latestKnowledge.isLiked ? <HeartFilled style={{ color: '#ef4444' }} /> : <HeartOutlined />}
          onClick={() => toggleLike(latestKnowledge.id)}
        >
          {latestKnowledge.isLiked ? '已点赞' : '点赞'} {formatCount(latestKnowledge.likeCount)}
        </Button>
        <Button
          icon={latestKnowledge.isFavorited ? <StarFilled style={{ color: '#f59e0b' }} /> : <StarOutlined />}
          onClick={() => toggleFavorite(latestKnowledge.id)}
        >
        ...
        {latestKnowledge.status !== 'archived' && (
          <Button icon={<EditOutlined />} onClick={() => navigate(`/create?edit=${latestKnowledge.id}`)}>编辑</Button>
        )}
        {isAdmin && (
          <>
            {latestKnowledge.status === 'archived' ? (
              <Button icon={<UndoOutlined />} onClick={() => { unarchiveKnowledge(latestKnowledge.id); message.success('已解除归档'); }}>
                解除归档
              </Button>
            ) : latestKnowledge.status === 'published' && (
              <Button icon={<InboxOutlined />} onClick={() => { archiveKnowledge(latestKnowledge.id); message.success('已归档'); }}>
                归档
              </Button>
            )}
            ...
            {versions.map((v, idx) => (
              <div key={v.id} style={{
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button size="small" onClick={() => setVersionContentVisible(v)}>
                    查看内容
                  </Button>
  ...
  return (
    <div className="page-container">
      ...
      >
        <Tabs
          items={[
            {
              key: 'link',
              label: <span><LinkOutlined /> 链接分享</span>,
              children: (
                <div style={{ padding: '16px 0' }}>
                  <Input value={shareUrl} readOnly />
                  <Button type="primary" style={{ marginTop: 12 }} onClick={handleCopyLink}>
                    复制链接
                  </Button>
                </div>
              ),
            ...
            {
              key: 'qrcode',
              label: <span><QrcodeOutlined /> 二维码</span>,
              children: (
      ...
      {/* 版本内容查看 Modal */}
      <Modal
        title={`版本 ${versionContentVisible?.versionNumber || ''} 内容`}
        open={!!versionContentVisible}
        onCancel={() => setVersionContentVisible(null)}
        footer={null}
        width={700}
      >
